"""Frontend-visible Agent backend: commands, event stream, and local worker.

This module owns the public ``AgentCommand`` / ``AgentBackend`` contract, the
in-process Event Stream cursor buffer, the Channel Approval Port, and the
single worker thread that runs the existing synchronous Agent Loop.
...
- ``DEFAULT_EVENT_POLL_TIMEOUT_SECONDS`` (0.1): bound used by ``events()`` so a
  frontend ``KeyboardInterrupt`` is not stuck inside an unbounded wait.

The worker thread is an adapter only. Agent Loop, Executor, Policy, and Event
Emitter behaviour is unchanged. Cross-thread shared state is limited to the
Event Stream buffer, the approval channel, and ``CancellationSignal``.
"""
...
class ChannelApprovalPort:
    """``ApprovalPort`` that publishes ``approval_request`` and waits for the frontend.
...
class LocalAgentBackend:
    """In-process ``AgentBackend`` with one worker thread running the Loop."""
