...
- 开发过程文档已统一迁移到 `docs/baseline/`，并于 2026-08-28 通过用户变更审阅；已完成 T01～T06 的范围和实现证据保持不变。
- 仓库现有 `pyproject.toml`、`src/coding_agent_neo/`、测试分层目录、示例配置和开发 README；CLI 目前只提供明确标注为未实现的公共帮助入口，模型和 Agent Loop 行为仍未实现；工具系统已完成 T04，策略/执行器已完成 T05，事件/Session Store 已完成 T06，T07 尚未实现。
- T01、T02、T03、T04、T05、T06 均已通过各自专用 worker 验证和主 Agent 独立复验；T07 依赖就绪但未派发。
...
- T07（OpenAI-compatible 模型访问与有界重试）是下一推荐任务，但尚未获得派发授权；本轮按用户要求止于 T06。
