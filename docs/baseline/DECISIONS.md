...
## 2026-08-31 — 基线实现不包含介绍视频录制

- 选择：`docs/baseline/` 约束的是基线版本，不是最终竞赛提交包。T13 只交付 `README.txt` 与演示脚本初稿，允许后续迭代；录制介绍/演示视频、产出 mp4、`ffprobe` 时长/大小检查、含视频的提交 zip 均不在本基线范围，也不是 T13 的通过条件。
- 理由：用户明确指令优先于仓库文档。题目原始材料仍要求最终提交含 2 分钟以内 mp4，但那属于后续最终版本；把尚未录制的视频当作基线验收项，会让未完成工作被写成阻塞或假完成。
- 拒绝或推迟：不改写 `docs/requirement.md` 或需求正文来假装从未要求视频，以免历史漂移。最终视频、姓名.zip 和截止后停推等竞赛提交动作推迟到后续迭代，且仍须用户明确执行或授权。
- 后果：架构升到 0.4，第 1.3 节增加基线排除项；T13 标题、范围、验收和验证去掉 mp4/`ffprobe`；`AGENTS.md` 交付物最低验证改为 README 字数与演示脚本审阅。本条部分 supersede 同日 T12 决策中“T13 再写面向用户的 README.txt/视频”的交付组合。
