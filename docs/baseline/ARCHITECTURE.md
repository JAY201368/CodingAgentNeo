...
1. CLI 解析配置并创建共享服务、root `AgentRuntime` 和 `LocalExecutionEnvironment`。
2. Environment `start()` 成功后发布 `session_start`、`agent_start`；用户输入形成 `user_message` 并立即持久化。
3. Context Builder 只投影当前 `agent_id` 的有效历史；预算超阈值时先执行当前 Runtime 专属 compaction。
...
2. Tool Executor 校验工具是否注册且激活、arguments JSON 与 schema 是否有效，并发布 `tool_call`。
...
- Compactor 保留 system prompt、必要工作区信息和最近完整交互，不拆散 assistant tool call 与 tool result；较早内容和旧 summary 生成增量 summary，持久化覆盖到的 sequence。
...
| 计划模块                     | 拥有的职责                                        | 禁止拥有或依赖                                    |
| ------------------------ | -------------------------------------------- | ------------------------------------------ |
| `cli.py` / `__main__.py` | 参数、交互输入、approval 询问、组装依赖、退出码                 | Agent 决策、路径安全、直接 JSONL 拼写                  |
...
| `tools/`                 | Tool schema、参数校验、注册/激活、内置工具语义                | `open()`、`Path` 实际读写、`subprocess`、宿主机 `rg` |
...
| `context.py`             | 当前 Agent 的上下文投影、预算估算、完整交互分组                  | 删除/改写 Session History、访问其他 Agent 内部消息      |
...
依赖方向必须满足：`Tool -> ExecutionEnvironment Protocol`；`Agent Loop -> 抽象服务 + AgentRuntime`；`Event producers -> EventEmitter -> Store/Renderer`。只有 `LocalExecutionEnvironment` 可以包含宿主机文件和进程实现。
...
Tool 至少公开 `name`、`description`、JSON-compatible schema、参数校验和 `execute(arguments, ToolExecutionContext)`。内置 active tools 为 `read_file`、`list_files`、`search`、`write_file`、`edit_file`、`bash`。默认策略：工作区内结构化文件工具 allow；交互 bash ask；越界或不安全参数 deny；auto/yolo 下 bash allow；策略异常 deny。
...
| 层级   | 真实依赖                                         | 主要证明内容                                   | 限制                        |
| ---- | -------------------------------------------- | ---------------------------------------- | ------------------------- |
| 单元测试 | fake clock/model/environment、临时目录            | 数据不变量、错误分类、路径边界、输出截断、Runtime 隔离          | 不能证明真实网关兼容或宿主 shell 隔离    |
| 组件测试 | `LocalExecutionEnvironment` + 临时 workspace   | 六类环境操作、symlink 逃逸、rg 检测、timeout/cancel   | 只代表当前 OS 权限模型             |
| 集成测试 | scripted fake model + fake/local environment | 完整 Loop、工具失败修正、事件顺序、预算、compaction、resume | fake model 不证明真实模型行为      |
| 验收测试 | 小型缺陷仓库 + 用户提供的测试 API 配置                      | AC-01 真实编程闭环和终端体验                        | 必须明确记录模型、环境和未执行项；不得提交 key |
| 静态审查 | `rg`/代码审查                                    | Tool 无宿主机 I/O、无全局 Runtime、无 secret、依赖方向  | 不能替代动态安全测试                |
...
| FR-31～FR-34 扩展边界         | 4、5         | T02、T03、T05、T06、T08 | AC-11～AC-14             |
