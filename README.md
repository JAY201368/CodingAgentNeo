...
With non-interactive `ask`, write operations and bash are immediately denied and stdin is never consumed for approval; choose `--approval-mode auto` or `--yolo` for explicitly unattended writes and bash. Never put a key value in source, arguments, tracked configuration, or session files. `--resume <session_id>` continues a linear session without replaying historical tool side effects.
